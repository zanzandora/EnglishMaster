// report.tsx
import { useExporReport } from 'hooks/useExportExcel';
import React from 'react';
interface ExportReportToExcelProps {
  selectedReport: string;
  filteredReports: any[];
  getScoreGrade: (score: number) => string;
}

const ExportReportToExcel: React.FC<ExportReportToExcelProps> = ({
  selectedReport,
  filteredReports,
  getScoreGrade,
}) => {
  const { handleExport } = useExporReport();

  return (
    // <div>
    //   <button
    //     onClick={() =>
    //       handleExport(selectedReport, filteredReports, getScoreGrade)
    //     }
    ...
    <div>
      <div className='relative flex justify-center items-center'>
        <button
          ...
          className='bg-green-600 text-white py-2 px-8 rounded-lg shadow-md hover:bg-green-700 focus:outline-none flex items-center'
        >
          <svg
            fill='#fff'
