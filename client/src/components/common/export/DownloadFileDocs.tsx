...
interface DownloadFileDocsProps {
  fileUrl: string;
  fileName: string;
  type: string;
}

const DownloadFileDocs: React.FC<DownloadFileDocsProps> = ({
  fileUrl,
  fileName,
  type,
}) => {
  const handleDownloadFile = () => {
    ...
    const downloadUrl = `/${type}/download/${encodeURIComponent(key)}`;

    // Tạo link tải ẩn để xử lý download
...
export default DownloadFileDocs;
